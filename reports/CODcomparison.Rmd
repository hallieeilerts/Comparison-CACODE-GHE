...
subtitle: "Scatter plots and trends 2000-2019"
output:
  html_document:
    toc: true
    toc_float: true
...
p %>% filter(Name == "World") %>%
  ...
  labs(title = "5-9y 2000-2019, World") +
  theme_bw() +
...
```{r tab5to9collectVio, eval = FALSE}
...
dat %>%
  ...
  filter(AgeGroup %in% c("05to09y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
  mutate(CACODE = frac_cacode, GHE = frac_ghe) %>%
  filter(COD == "CollectVio"  & GHE > 0.25 & CACODE < 0.1) %>%
  select(CountryName, Year,GHE,CACODE) %>%
  arrange(CountryName, Year) %>%
  mutate(Year = as.character(Year)) %>%
  mutate(GHE= paste0(formatC(GHE*100, 1, format="f")),
         CACODE = paste0(formatC(CACODE*100, 1, format="f"))) %>%
  flextable() %>% align(j=3:4, align="center", part="all") %>%
  merge_v(., j = ~ CountryName) %>%
  hline(., part = "all", border = std_border) %>%
  set_header_labels(., Name = "ISO3") %>%
  fontsize(size = 8, part = "all") 
```

Countries with large differences in CollectVio (CSMFs for GHE > 0.25 and CACODE < 0.10), all causes

```{r bar5to9collectVio,  fig.height = 9, fig.width=9}
dat %>%
  filter(!(COD %in% c("COVID19", "OtherCOVID19"))) %>%
  filter(AgeGroup %in% c("05to09y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
  mutate(CACODE = frac_cacode, GHE = frac_ghe) %>%
  mutate(ind = ifelse(COD == "CollectVio"  & GHE > 0.25 & CACODE < 0.1, 1, 0)) %>%
...
```{r trend5to9hiv,  fig.height = 8, fig.width=10}
dat %>%
  filter(AgeGroup %in% c("05to09y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
  mutate(CACODE = frac_cacode, GHE = frac_ghe) %>%
...
```{r bar5to9hiv,  fig.height = 9, fig.width=9}
dat %>%
  filter(!(COD %in% c("COVID19", "OtherCOVID19"))) %>%
  filter(AgeGroup %in% c("05to09y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
...
```{r trend5to9malaria,  fig.height = 8, fig.width=10}
dat %>%
  filter(AgeGroup %in% c("05to09y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
  mutate(CACODE = frac_cacode, GHE = frac_ghe) %>%
...
```{r bar5to9mal,  fig.height = 9, fig.width=9}
dat %>%
  filter(!(COD %in% c("COVID19", "OtherCOVID19"))) %>%
  filter(AgeGroup %in% c("05to09y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
...
```{r bar10to14malaria,  fig.height = 9, fig.width=9}
dat %>%
  filter(!(COD %in% c("COVID19", "OtherCOVID19"))) %>%
  filter(AgeGroup %in% c("10to14y") & Sex == "Total" & 
           AdminLevel %in% c("National")) %>%
...
```{r trend10to14RegOtherInj,  fig.height = 6, fig.width=10, eval = FALSE}
## OtherInj
# National values over time by region
...
p %>%
  ...
  ggplot()  +
  geom_point(aes(x=Year,y=value, col = Source), alpha = .3, shape = 1) +
  geom_smooth(aes(x=Year,y=value, col = Source)) +
  labs(title = "10-14y", y= "CSMF") +
  facet_wrap(~SDGregion, ncol = 5) +
  scale_color_manual(values = c("#8C2981FF","#FA7F5EFF"), name = "")
```
...
Countries largest average difference in Materna

```{r trend15to19Fmat,  fig.height = 8, fig.width=10}
dat %>%
  filter(AgeGroup %in% c("15to19y") & Sex == "Female" & 
           AdminLevel %in% c("National")) %>%
  mutate(CACODE = frac_cacode, GHE = frac_ghe) %>%
...
```{r bar15to19Fmat,  fig.height = 9, fig.width=9}
dat %>%
  filter(!(COD %in% c("COVID19", "OtherCOVID19"))) %>%
  filter(AgeGroup %in% c("15to19y") & Sex == "Female" & 
           AdminLevel %in% c("National")) %>%
...
```{r bar15to19Mcolvio,  fig.height = 9, fig.width=9}
dat %>%
  filter(!(COD %in% c("COVID19", "OtherCOVID19","Maternal"))) %>%
  filter(AgeGroup %in% c("15to19y") & Sex == "Male" & 
           AdminLevel %in% c("National")) %>%
